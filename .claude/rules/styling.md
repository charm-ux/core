...
- **STYLE-008** — Set runtime-computed values (measured dims, dynamic percentages) with `this.style.setProperty()` + `var()` fallback; never set presentation (`color`, `background`) that way. ([details](../../.agents/rules/styling/STYLE-008.md))
- **STYLE-009** — Follow the ADR-0001 token naming grammar: nest states, `gap` for between-element spacing, `bgColor` for backgrounds, `fgColor` for the generic component foreground, `color` for text-scoped element groups (label/placeholder/helpText/message), `iconColor` for icon-scoped colors, never `backgroundColor`/`foregroundColor`. ([details](../../.agents/rules/styling/STYLE-009.md))
