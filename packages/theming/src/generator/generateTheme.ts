...
>(tokens: TokenSource<P, S, C>, options: GenerateThemeOptions): { cfg: ResolvedConfig; result: GenerateThemeResult } {
  ...
  // Generate theme CSS
  let themeCss = generateCss(definition as TokenDefinition | ResolvedTokenDefinition, cfg);
  if (definition.rawCss?.theme) {
    themeCss += `\n/* Custom theme CSS */\n${definition.rawCss.theme}`;
  }

  // Generate CSS reset
  let resetCss = cfg.skipReset
    ? undefined
    : `/* Generated by @charm-ux/theming - DO NOT EDIT */\n${generateReset({ prefix, useLayers: cfg.useLayers })}`;
  if (resetCss && definition.rawCss?.reset) {
    resetCss += `\n/* Custom reset CSS */\n${definition.rawCss.reset}`;
  }

  // Generate utilities
  let utilitiesCss = cfg.skipUtilities
    ? undefined
    : generateUtilities(definition.primitives, { prefix, useLayers: cfg.useLayers });
  if (utilitiesCss && definition.rawCss?.utilities) {
    utilitiesCss += `\n/* Custom utility CSS */\n${definition.rawCss.utilities}`;
  }
