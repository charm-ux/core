...
To create an icon-only button, leave the button text blank and put the icon in the default slot. When the default slot contains only an icon — an `<svg>`, the `<ch-icon>` component, or an element with an `icon` attribute — icon-only spacing is applied automatically. A visually-hidden label doesn't affect detection, so prefer it over relying on the icon's own label:
...
You can also force icon-only treatment with the `icon-only` attribute, which takes precedence over detection.
