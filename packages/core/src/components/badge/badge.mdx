...
Use the `start` and `end` slots to place an element, such as an icon, before or after the badge content. A badge is a suitable component to place a compact icon before the content of a label.

<code-bubble>

```html
<ch-badge>
  <ch-icon slot="start" name="checkmark-circle"></ch-icon>
  Success
</ch-badge>
```
